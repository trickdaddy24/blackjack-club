...
/** GET: my pending Spades invites (expiry enforced lazily on read) —
 *  mirrors /api/invites for the blackjack Duo table. */
export async function GET() {
  ...
  await prisma.spadesInvite.updateMany({
    where: { toId: session.user.id, status: "pending", expiresAt: { lt: new Date() } },
    data: { status: "expired" },
  });

  const invites = await prisma.spadesInvite.findMany({
